...
def plotPredictions(safety_learned, data_episode, savename):
    ...
    drift_inputs, _, us, residuals, _, bpredst, _, _, _, _  = data_episode
    test_inputs = (drift_inputs - np.repeat(np.reshape(safety_learned.preprocess_mean, (-1, 8)), drift_inputs.shape[0], 0))
    test_inputs = np.divide(test_inputs, np.repeat(np.reshape(safety_learned.preprocess_std, (-1, 8)), drift_inputs.shape[0], 0))
    ...
    with torch.no_grad(), gpytorch.settings.fast_computations(solves=False):
        residual_pred = likelihood(safety_learned.residual_model(test_inputs_tensor.cpu()))

    lower, upper = residual_pred.confidence_region()
    lower = lower.cpu()
    upper = upper.cpu()
    mean = residual_pred.mean.cpu().numpy()
...
class LearnedSegwaySafetyAAR(LearnedAffineDynamics):
    ...
    def __init__(self, segway_est):
        ...
        self.input_data = []
        self.preprocess_mean = np.zeros((8,))
    ...
    def drift_learned(self, x, t):
        ...
        """
        xfull = np.concatenate(([1.0],np.divide(self.process_drift(x, t) - self.preprocess_mean, self.preprocess_std)))
        xfull = np.reshape(xfull, (-1, 9))
        xfull_tensor = torch.from_numpy(xfull).float()

        cross1 = self.residual_model.k1(xfull_tensor, self.input_data).numpy()/self.usstd
        cross2 = self.residual_model.k2(xfull_tensor, self.input_data).numpy()
        bmean = np.dot(cross2, self.alpha)
        amean = np.dot(cross1, self.alpha)
        
        mean = bmean*self.residual_std
        variance = self.residual_model.k2(xfull_tensor, xfull_tensor).numpy() - np.dot( np.dot(cross2, self.Kinv), cross2.T )
        varab = -np.dot(np.dot(cross1, self.Kinv), cross2.T)
        return [self.dynamics.drift(x, t) + mean.ravel() + self.residual_mean + self.comp_safety(self.eval(x,t)), variance.ravel(), varab.ravel()]
    
    def act_learned(self, x, t):
        """
          Find mean and variance of control-dependent dynamics a after residual modeling.
        """
        xfull = np.concatenate(([1.0],np.divide(self.process_act(x, t) - self.preprocess_mean, self.preprocess_std)))
        xfull = np.reshape(xfull,(-1, 9))
        xfull_tensor = torch.from_numpy(xfull).float()
        cross = self.residual_model.k1(xfull_tensor, self.input_data).numpy()/self.usstd
        mean = np.dot(cross, self.alpha)*self.residual_std
        variancequad = self.residual_model.k1(xfull_tensor, xfull_tensor).numpy()/(self.usstd)**2 - np.dot( np.dot(cross, self.Kinv), cross.T)
...
def run_experiment(rnd_seed, num_episodes, num_tests, save_dir):
  ...
  phi_0 = lambda x, t: safety_learned.drift_estimate( x, t )+comp_safety(safety_learned.eval(x,t))
  ...
  for i in range(num_episodes):
      ...
      if(i>0):
        drift_est, drift_true, drift_learned, act_est, act_true, act_learned, drift_var, act_var, res_expected = postProcessEpisode(xso, uso, ts_qp, safety_est, safety_true, safety_learned, startpoint)
      ...
      residual_mean = 0
      residual_std = 1
      residuals = np.reshape((residuals-residual_mean)/residual_std,(residuals.size,-1))+0.01*np.random.randn(residuals.size,1)
      ...
      print("kerner scale for a(x)",residual_model.covar_module.kernels[0].kernels[1].outputscale.item())
      print("kerner scale for u",residual_model.covar_module.kernels[0].kernels[0].outputscale.item())
      print("kernel lengthscale for b(x)",residual_model.covar_module.kernels[1].base_kernel.lengthscale)
      print("kerner scale for b(x)",residual_model.covar_module.kernels[1].outputscale.item())
      ...
      safety_learned.residual_std = residual_std
      safety_learned.residual_mean = residual_mean
      ...
      safety_learned.Kinv = np.linalg.pinv(residual_model.covar_module(input_data).numpy()+residual_model.likelihood.noise.item()*np.eye(input_data.shape[0]))

      safety_learned.alpha = np.dot(safety_learned.Kinv,residuals)

      safety_learned.input_data = input_data
      safety_learned.preprocess_mean = preprocess_mean[0]
      safety_learned.preprocess_std = preprocess_std[0]
